	...
	function Compiler(renderer) {
	  ...
	  this.compile = function(source, name) {
		...
		function filterAST(ast) {
			var context = {};
			return _this.filterNode(context, ast);
		}

		this.filterNode = function(context, node) {
			return _this.optimizers[node[0]](context, node);
		};
		...
		function visit(context, node) {
			...
			for (i=1, len=node.length; i<len; i++) {
				res = _this.filterNode(context, node[i]);
				if (res) {
					out.push(res);
				}
			}
		...
		function format(context, node) {
			if(_this.renderer.config.whitespace) {
		...
		function compile(ast, name) {
			...
			entry = _this.compileNode(context, ast),
			iife;
			...
			if(_this.renderer.config.amd) {
				return 'define(' + AMDName + '["dust.core"],' + compiled + ');';
			} else if(_this.renderer.config.cjs) {
		...
		function compileParts(context, body) {
			...
			for (i=1, len=body.length; i<len; i++) {
				parts += _this.compileNode(context, body[i]);
			}
			return parts;
		}

		this.compileNode = function(context, node) {
			return _this.nodes[node[0]](context, node);
		};
		...
		this.nodes = {
			...
			reference: function(context, node) {
				return '.f(' + _this.compileNode(context, node[1]) +
					',ctx,' + _this.compileNode(context, node[2]) + ')';
			},
			...
			'<': function(context, node) {
				...
				for (var i=1, len=bodies.length; i<len; i++) {
					var param = bodies[i],
							type = param[1][1];
					if (type === 'block') {
						context.blocks[node[1].text] = _this.compileNode(context, param[2]);
						return '';
					}
				}
				return '';
			...
			'+': function(context, node) {
				if (typeof(node[1].text) === 'undefined'  && typeof(node[4]) === 'undefined'){
					return '.b(ctx.getBlock(' +
								_this.compileNode(context, node[1]) +
								',chk, ctx),' + _this.compileNode(context, node[2]) + ', {},' +
								_this.compileNode(context, node[3]) +
								')';
				} else {
					return '.b(ctx.getBlock(' +
							escape(node[1].text) +
							'),' + _this.compileNode(context, node[2]) + ',' +
							_this.compileNode(context, node[4]) + ',' +
							_this.compileNode(context, node[3]) +
							')';
				}
			},
			...
			'@': function(context, node) {
				return '.h(' +
					...
					_this.compileNode(context, node[4]) + ',' +
					_this.compileNode(context, node[3]) + ',' +
					_this.compileNode(context, node[5]) +
					')';
			},
			...
			'%': function(context, node) {
				...
				return _this.pragmas[name](context, ctx, bodies, params);
			},
			...
			context: function(context, node) {
				if (node[1]) {
					return 'ctx.rebase(' + _this.compileNode(context, node[1]) + ')';
				}
				return 'ctx';
			},
			...
			params: function(context, node) {
				...
				for (var i=1, len=node.length; i<len; i++) {
					out.push(_this.compileNode(context, node[i]));
				}
				if (out.length) {
					return '{' + out.join(',') + '}';
				}
			...
			param: function(context, node) {
				return _this.compileNode(context, node[1]) + ':' + _this.compileNode(context, node[2]);
			},

			filterargs: function(context, node) {
				var list = [];
				for (var i=1, len=node.length; i<len; i++) {
					list.push(_this.compileNode(context, node[i]));
				}
				if (list.length) {
					return list.join(',');
				}
			...
			filters: function(context, node) {
				var list = [];
				for (var i=1, len=node.length; i<len; i++) {
					var filterdef = node[i];
					var args = _this.compileNode(context, filterdef.args);

					// a filter with args is an array - [filtername, arg1, arg2, arg3 ...]
					// a filter without args is a string "filtername" (same as original dustjs)
					if (args.length) {
						list.push('["' + filterdef.name + '"' + (args.length ? ',' + args : '') + ']');
					} else {
						list.push('"' + filterdef.name + '"');
					}
				}
				return '"' + context.auto + '"' +
			...
			path: function(context, node) {
				...
				for (var i=0,len=keys.length; i<len; i++) {
					if (isArray(keys[i])) {
						list.push(_this.compileNode(context, keys[i]));
					} else {
						list.push('"' + keys[i] + '"');
					}
				}
		...
		function renderSource(source, context, callback) {
			var tmpl = _this.renderer.loadSource(_this.compile(source));
			return loaderFor(tmpl)(context, callback);
		}

		function compileFn(source, name) {
			var tmpl = _this.renderer.loadSource(_this.compile(source, name));
			return loaderFor(tmpl);
		}

		function loaderFor(tmpl) {
			var dust = _this.renderer; // use a local dust, set to the renderer for cjs compat
		...
		this.renderer.compile = _this.compile;
		this.renderer.renderSource = renderSource;
		this.renderer.compileFn = compileFn;

		// DEPRECATED legacy names. Removed in 2.8.0
		this.renderer.filterNode = _this.filterNode;
		this.renderer.optimizers = _this.optimizers;
		this.renderer.pragmas = _this.pragmas;
		this.renderer.compileNode = _this.compileNode;
		this.renderer.nodes = _this.nodes;
